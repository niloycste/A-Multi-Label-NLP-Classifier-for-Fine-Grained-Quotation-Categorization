...
def index():
    if request.method == "POST":
        ...
        labels = [elem['label'] for elem in confidence_list if elem['confidence'] >= 0.10 ]
        label_text = ""
...
def predict_genres(input_text):
    response = requests.post("https://niloycste68-multi-label-quotes-classifier.hf.space/run/predict", json={
        "data": [
            input_text
        ]
    }).json()
